...
type Project = { id: string; title: string; stage: string; progress: number; next_milestone: string; client_due_at: string | null; payment_state: string; updated_at: string; budget_cents: number; currency: string }
...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    ...
    morphSupabaseQuery<Project>("morph_projects", token, { select: "id,title,stage,progress,next_milestone,client_due_at,payment_state,updated_at,budget_cents,currency", order: "updated_at.desc", limit: 100 })
  ...
  return <>
    ...
    <section className="py-7">{error&&<p role="alert" className="mb-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">{error}</p>}{!loading&&projects.length===0?<div className="rounded-2xl border border-black/10 bg-white p-8"><p className="text-[10px] uppercase tracking-[0.16em] text-black/35">Live registry</p><h2 className="mt-3 text-2xl font-semibold tracking-[-0.04em]">No projects yet.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-black/45">The first qualified client project will appear here after provisioning. No demo projects are shown.</p></div>:<div className="overflow-hidden rounded-2xl border border-black/10 bg-white">{projects.map((project,index)=><article key={project.id} className={`grid gap-4 p-5 lg:grid-cols-[1.1fr_0.65fr_0.7fr_1.15fr] lg:items-center ${index>0?"border-t border-black/10":""}`}><div><h2 className="font-semibold tracking-[-0.025em]">{project.title}</h2><p className="mt-2 text-xs text-black/40">Next · {project.next_milestone || "Not assigned"}</p><p className="mt-1 text-xs font-medium">{money(Number(project.budget_cents||0),project.currency)}</p></div><div><div className="flex justify-between text-[9px] uppercase tracking-[0.13em] text-black/40"><span>{project.stage}</span><span>{project.progress}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-black/[0.07]"><div className="h-full rounded-full bg-[#6257e8]" style={{width:`${project.progress}%`}} /></div></div></div><div><p className="text-[9px] uppercase tracking-[0.13em] text-black/35">Payment gate</p><p className="mt-2 text-xs font-medium">{project.payment_state}</p></div><div className="grid gap-2"><PaymentLinkButton projectId={project.id} kind="deposit"/><PaymentLinkButton projectId={project.id} kind="balance"/></div></article>)}</div>}</section>
  </>
}
