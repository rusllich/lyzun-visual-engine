...
export async function POST(request: Request) {
  try {
    ...
    const nextStatus = paid ? "confirmed" : failed ? "failed" : "confirming"
...
export async function GET(request: Request) {
  ...
  const token = url.searchParams.get("token") || request.headers.get("x-morph-payoneer-token") || ""
  const expected = process.env.PAYONEER_NOTIFICATION_SECRET?.trim() || ""
  if (!expected || !token || !safeEqual(token, expected)) return NextResponse.json({ error: "Unauthorized" }, { status: 401 })

  const payload: Json = Object.fromEntries(url.searchParams.entries())
  delete payload.token
  return POST(new Request(request.url, { method: "POST", headers: { "content-type": "application/json", "x-morph-payoneer-token": expected }, body: JSON.stringify(payload) }))
}
