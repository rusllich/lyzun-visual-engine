...
function notificationSecret() {
  const value = process.env.PAYONEER_NOTIFICATION_SECRET?.trim()
  if (!value) throw new Error("PAYONEER_NOTIFICATION_SECRET is not configured")
  return value
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!input?.projectId || !input.kind || !["deposit", "balance"].includes(input.kind)) return NextResponse.json({ error: "projectId and payment kind are required" }, { status: 400 })
    ...
    const project = await morphServerSelectOne<Project>("morph_projects", accessToken, { id: input.projectId }, "id,client_id,title,budget_cents,currency")
    if (!project) return NextResponse.json({ error: "Project not found" }, { status: 404 })

    const existing = await findActivePayment(accessToken, project.id, input.kind)
    if (existing) {
      if (existing.provider === "payoneer" && existing.provider_checkout_url) return NextResponse.json({ paymentId: existing.id, provider: "payoneer", checkoutUrl: existing.provider_checkout_url, amountCents: Number(existing.amount_cents), currency: existing.currency, reused: true })
    ...
    const client = await morphServerSelectOne<Client>("morph_clients", accessToken, { id: project.client_id }, "id,display_name,company_name,email")
    ...
    try {
      await morphServerInsert<Payment>("morph_payments", accessToken, { id: paymentId, project_id: project.id, provider: "payoneer", provider_payment_id: null, kind: input.kind, amount_cents: amountCents, currency: project.currency.toUpperCase(), status: "pending", provider_payment_status: "creating", provider_checkout_url: null })
    } catch (error) {
      const raced = await findActivePayment(accessToken, project.id, input.kind)
      if (raced?.provider === "payoneer" && raced.provider_checkout_url) return NextResponse.json({ paymentId: raced.id, provider: "payoneer", checkoutUrl: raced.provider_checkout_url, amountCents: Number(raced.amount_cents), currency: raced.currency, reused: true })
      throw error
    }
    ...
    try {
      const session = await createPayoneerHostedSession({
        ...
        country,
        channel: "WEB_ORDER",
        payment: { amount, currency: project.currency.toUpperCase() },
        products: [{ name: `MORPH · ${project.title} · ${input.kind}`, type: "DIGITAL", amount, quantity: 1 }],
        callback: {
          ...
          notificationHeaders: [{ name: "X-Morph-Payoneer-Token", value: notificationSecret() }],
        },
      ...
      await morphServerPatch("morph_payments", accessToken, { id: paymentId }, { status: "failed", provider_payment_status: "session_creation_failed", updated_at: new Date().toISOString() }).catch(() => null)
      throw error
    }
  } catch (error) {
