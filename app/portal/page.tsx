...
type Payment = { id: string; project_id: string; kind: string; amount_cents: number; currency: string; status: string; provider_payment_status: string | null; provider_checkout_url: string | null; created_at: string }
...
function settled(payment:Payment){return /paid|finished|confirmed|verified/i.test(`${payment.status} ${payment.provider_payment_status||""}`)}
...
export default function ClientPortalPage() {
  ...
  useEffect(() => {
    ...
    async function restorePortal(accessToken: string) {
      try {
        ...
        const paymentRows = currentClient ? await morphSupabaseQuery<Payment>("morph_payments", accessToken, { select: "id,project_id,kind,amount_cents,currency,status,provider_payment_status,provider_checkout_url,created_at", order: "created_at.desc" }) : []
        setToken(accessToken); setClient(currentClient); setProjects(projectRows); setMilestones(milestoneRows); setPayments(paymentRows); setError("")
      } catch (cause) {
  ...
  async function loadPortal(accessToken: string) {
    try {
      ...
      if (!currentClient) { setProjects([]); setMilestones([]); setPayments([]); return }
      ...
      const paymentRows = await morphSupabaseQuery<Payment>("morph_payments", accessToken, { select: "id,project_id,kind,amount_cents,currency,status,provider_payment_status,provider_checkout_url,created_at", order: "created_at.desc" })
      setProjects(projectRows); setMilestones(milestoneRows); setPayments(paymentRows); setError("")
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Could not load your workspace")
    } finally { setLoading(false) }
  }
  ...
  function signOut() {
    sessionStorage.removeItem(TOKEN_KEY)
    setToken(null); setClient(null); setProjects([]); setMilestones([]); setPayments([]); setError("")
  }
